...
private:
  ...
  std::mutex mutable mutex_new_msg_;
  bool running_;
...
public:
  ...
  void start_server(int port, int period, const std::string &topic) {
    ...
    recv_socket_.bind("tcp://*:" + std::to_string(port + 1));
  ...
  mxArray *getLatestArr() {
    ...
    std::lock_guard<std::mutex> lock(mutex_new_msg_);
    ...
    switch (latest_msg_.type()) {
      ...
      case (MatlabPython::Type::UINT8): {
        latest_arr = mxCreateNumericArray(ndim, dims, mxClassID::mxUINT8_CLASS, mxREAL);
        double *ptr = mxGetPr(latest_arr);
        memcpy(ptr, latest_msg_.data().data(), total_elements * sizeof(char));
        break;
      }
      default: {
  ...
  void send(const mxArray *pMxArray) {
    ...
    switch (id) {
      ...
      case mxClassID::mxUINT8_CLASS: {
        msg.set_type(MatlabPython::Type::UINT8);
        int *data_ptr = (int *) mxGetData(pMxArray);
        msg.set_data(reinterpret_cast<void *>(data_ptr), number_elements * sizeof(char));
        break;
      }
      default:
        std::stringstream ss;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  switch (command) {
    ...
    case HAS_NEW_MSG: {
      if (nrhs != 2)
        mexErrMsgTxt("function takes three inputs: [command, ptr]");
      if (nlhs != 1)
        mexErrMsgTxt("hasNewMsg function has one outputs");

      Server *server = getServer(prhs);
      plhs[0] = mxCreateDoubleScalar(server->has_new_msg_);
      break;
    }
